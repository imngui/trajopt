...
struct CartPoseErrCalculator : public VectorOfVector {
  ...
  KinBodyInterface::LinkPtr link_;
  CartPoseErrCalculator(const Eigen::Isometry3d& pose, ConfigurationPtr manip, KinBodyInterface::LinkPtr link) :
    pose_inv_(pose.inverse()),
    manip_(manip),
    link_(link) {}
  VectorXd operator()(const VectorXd& dof_vals) const override;
};
...
//   void Plot(const DblVec& x, EnvironmentInterface& env, std::vector<std::shared_ptr<void>>& handles) override;
// };
...
struct CartVelJacCalculator : MatrixOfVector {
  ...
  KinBodyInterface::LinkPtr link_;
  double limit_;
  CartVelJacCalculator(ConfigurationPtr manip, KinBodyInterface::LinkPtr link, double limit) :
    manip_(manip), link_(link), limit_(limit) {}

  MatrixXd operator()(const VectorXd& dof_vals) const override;
};
...
struct CartVelCalculator : VectorOfVector {
  ...
  KinBodyInterface::LinkPtr link_;
  double limit_;
  CartVelCalculator(ConfigurationPtr manip, KinBodyInterface::LinkPtr link, double limit) :
    manip_(manip), link_(link), limit_(limit) {}
