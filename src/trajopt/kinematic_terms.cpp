...
inline Vector3d rotVec(const Eigen::Quaterniond& q) {
...
VectorXd CartPoseErrCalculator::operator()(const VectorXd& dof_vals) const {
  ...
  Eigen::Isometry3d newpose = link_->GetTransform();
  Eigen::Isometry3d pose_err = pose_inv_ * newpose;
  VectorXd err(6);
  err.head<3>() = rotVec(Eigen::Quaterniond(pose_err.rotation()));
  err.tail<3>() = pose_err.translation();
  return err;
}

// void CartPoseErrorPlotter::Plot(const DblVec& x, EnvironmentInterface& env, std::vector<std::shared_ptr<void>>& handles) {
//   auto calc = static_cast<CartPoseErrCalculator*>(m_calc.get());
//   DblVec dof_vals = getDblVec(x, m_vars);
//   calc->manip_->SetDOFValues(dof_vals);
//   Eigen::Isometry3d target = calc->pose_inv_.inverse();
//   Eigen::Isometry3d cur = calc->link_->GetTransform();
//   env.PlotAxes(cur, 0.05, handles);
//   env.PlotAxes(target, 0.05, handles);
//   handles.push_back(env.drawArrow(cur.translation(), target.translation(), 0.005, Eigen::Vector4d(1, 0, 1, 1)));
// }
...
MatrixXd CartVelJacCalculator::operator()(const VectorXd& dof_vals) const {
  ...
  Eigen::Isometry3d pose0 = link_->GetTransform();
  MatrixXd jac0 = manip_->PositionJacobian(link_->GetIndex(), pose0.translation());
  manip_->SetDOFValues(toDblVec(dof_vals.bottomRows(n_dof)));
  Eigen::Isometry3d pose1 = link_->GetTransform();
  MatrixXd jac1 = manip_->PositionJacobian(link_->GetIndex(), pose1.translation());
...
VectorXd CartVelCalculator::operator()(const VectorXd& dof_vals) const {
  ...
  Eigen::Isometry3d pose1 = link_->GetTransform();
  VectorXd out(6);
  out.head<3>() = pose1.translation() - pose0.translation() - Eigen::Vector3d(limit_, limit_, limit_);
  out.tail<3>() = pose0.translation() - pose1.translation() - Eigen::Vector3d(limit_, limit_, limit_);
  return out;
}
