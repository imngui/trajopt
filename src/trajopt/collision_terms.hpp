...
typedef std::map<const KinBodyInterface::Link*, int> Link2Int;

struct CollisionEvaluator {
    virtual void CalcDistExpressions(const DblVec& x, std::vector<AffExpr>& exprs) = 0;
    virtual void CalcDists(const DblVec& x, DblVec& exprs) = 0;
    virtual void CalcCollisions(const DblVec& x, std::vector<Collision>& collisions) = 0;
    void GetCollisionsCached(const DblVec& x, std::vector<Collision>&);
    virtual ~CollisionEvaluator() {}
    virtual VarVector GetVars() = 0;

    Cache<size_t, std::vector<Collision>, 3> m_cache;
};
...
public:
    ...
    void CalcDistExpressions(const DblVec& x, std::vector<AffExpr>& exprs);
    void CalcDists(const DblVec& x, DblVec& exprs);
    void CalcCollisions(const DblVec& x, std::vector<Collision>& collisions);
    VarVector GetVars() { return m_vars; }

    EnvironmentInterface::Ptr m_env;
    CollisionChecker::Ptr m_cc;
    ConfigurationPtr m_rad;
    VarVector m_vars;
    Link2Int m_link2ind;
    std::vector<KinBodyInterface::LinkPtr> m_links;
    short m_filterMask;
};
...
public:
    ...
    virtual ConvexObjectivePtr convex(const std::vector<double>& x, Model* model);
    virtual double value(const std::vector<double>&);
...
public:
    ...
    virtual ConvexConstraintsPtr convex(const std::vector<double>& x, Model* model);
    virtual DblVec value(const std::vector<double>&);
    // void Plot(const DblVec& x, EnvironmentInterface& env, std::vector<std::shared_ptr<void>>& handles);
    VarVector getVars() { return m_calc->GetVars(); }
