...
void RobotAndDOF::SetRobotActiveDOFs() {
  std::vector<int> current_active = robot->GetActiveDOFIndices();
  if (robot->GetActiveDOF() != GetDOF() || !std::equal(current_active.begin(), current_active.end(), joint_inds.begin()))
...
void RobotAndDOF::GetDOFLimits(DblVec& lower, DblVec& upper) const {
  ...
  const int translation_dofs[3] = {0, 1, 2}; // X, Y, Z
  for (int i = 0; i < 3; ++i) {
  ...
  if (affinedofs & RotationMask) {
    ...
    } else throw std::invalid_argument("invalid rotation dofs");
  }
}
...
DblMatrix RobotAndDOF::RotationJacobian(int link_ind) const {
  ...
  const_cast<RobotAndDOF*>(this)->SetRobotActiveDOFs();
  std::vector<double> jacdata;
  robot->ComputeJacobianAxisAngle(link_ind, jacdata);
  return Eigen::Map<DblMatrix>(jacdata.data(), 3, GetDOF());
}

bool RobotAndDOF::DoesAffect(const KinBodyInterface::Link& link) {
...
std::vector<KinBodyInterface::LinkPtr> RobotAndDOF::GetAffectedLinks() {
  std::vector<int> inds;
  std::vector<KinBodyInterface::LinkPtr> out;
  GetAffectedLinks(out, false, inds);
  return out;
}

void RobotAndDOF::GetAffectedLinks(std::vector<KinBodyInterface::LinkPtr>& links, bool only_with_geom, std::vector<int>& link_inds) {
  links.clear();
  link_inds.clear();
  std::vector<KinBodyInterface::LinkPtr> all_links = GetRobot()->GetLinks();
  for (const auto& link : all_links) {
    if (this->DoesAffect(*link) && !(only_with_geom && !link->HasGeometries())) {
      links.push_back(link);
      link_inds.push_back(link->GetIndex());
    }
  }

  std::vector<KinBodyInterface::Ptr> grabbed;
  robot->GetGrabbed(grabbed);
  for (const auto& body : grabbed) {
    KinBodyInterface::LinkPtr grabberLink = robot->IsGrabbing(body);
    assert(grabberLink);
    for (const auto& link : body->GetLinks()) {
      if (link->HasGeometries()) {
        links.push_back(link);
...
std::vector<KinBodyInterface::Ptr> RobotAndDOF::GetBodies() {
  std::set<KinBodyInterface::Ptr> bodies;
  robot->GetAttached(bodies);
  return std::vector<KinBodyInterface::Ptr>(bodies.begin(), bodies.end());
}
...
DblVec RobotAndDOF::RandomDOFValues() {
  ...
  for (int i = 0; i < ndof; ++i) {
    lower[i] = std::max(lower[i], -2 * M_PI);
    upper[i] = std::min(upper[i], 2 * M_PI);
