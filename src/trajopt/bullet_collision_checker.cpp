...
public:
  CollisionObjectWrapper(KinBodyInterface::Link* link) : m_link(link), m_index(-1) {}
  vector<shared_ptr<void>> m_data;
  KinBodyInterface::Link* m_link;
  int m_index; 
  template<class T>
  void manage(T* t) { 
    m_data.push_back(shared_ptr<T>(t));
  }
  template<class T>
...
typedef shared_ptr<CollisionObjectWrapper> COWPtr;

inline const KinBodyInterface::Link* getLink(const btCollisionObject* o) {
...
btVector3 toBt(const Eigen::Vector3d& v){
  return btVector3(v[0], v[1], v[2]);
}
Eigen::Vector3d toEigen(const btVector3& v) {
  return Eigen::Vector3d(v.x(), v.y(), v.z());
}
btQuaternion toBtQuat(const Eigen::Quaterniond& q) {
  return btQuaternion(q.x(), q.y(), q.z(), q.w());
}
btTransform toBt(const Eigen::Isometry3d& t){
  return btTransform(toBtQuat(Eigen::Quaterniond(t.rotation())), toBt(t.translation()));
}

bool isIdentity(const Eigen::Isometry3d& T) {
  float e = 1e-6;
  return T.isApprox(Eigen::Isometry3d::Identity(), e);
}
...
btCollisionShape* createShapePrimitive(KinBodyInterface::Link::GeometryPtr geom, bool useTrimesh, CollisionObjectWrapper* cow) {
  ...
  case ShapeType::TriMesh: {
    const ShapeType::TriMesh &mesh = geom->GetCollisionMesh();
    assert(mesh.indices.size() >= 3);
    shared_ptr<btTriangleMesh> ptrimesh(new btTriangleMesh());
    ...
    if (useTrimesh) {
      ...
      convexTrimesh.setMargin(MARGIN); 

      bool useShapeHull;

      btShapeHull shapeHull(&convexTrimesh);
      if (mesh.vertices.size() >= 50) {
        bool success = shapeHull.buildHull(-666); 
...
COWPtr CollisionObjectFromLink(KinBodyInterface::LinkPtr link, bool useTrimesh) {
  LOG_DEBUG("creating bt collision object from from %s",link->GetName().c_str());

  const auto& geometries = link->GetGeometries();
...
void RenderCollisionShape(btCollisionShape* shape, const btTransform& tf,
    EnvironmentInterface& env, vector<shared_ptr<void>>& handles) {
...
class BulletCollisionChecker : public CollisionChecker {
  ...
  typedef map<const KinBodyInterface::Link*, CollisionObjectWrapper*> Link2Cow;
  Link2Cow m_link2cow;
  double m_contactDistance;
  vector<KinBodyInterface::Ptr> m_prevbodies;
  typedef std::pair<const KinBodyInterface::Link*, const KinBodyInterface::Link*> LinkPair;
  set< LinkPair > m_excludedPairs;
  Eigen::Matrix<bool, Eigen::Dynamic, Eigen::Dynamic> m_allowedCollisionMatrix;

public:
  BulletCollisionChecker(EnvironmentInterface::Ptr env);
  ...
  // virtual void PlotCollisionGeometry(vector<shared_ptr<void>>& handles);
  virtual void ExcludeCollisionPair(const KinBodyInterface::Link& link0, const KinBodyInterface::Link& link1) {
  ...
  virtual void IncludeCollisionPair(const KinBodyInterface::Link& link0, const KinBodyInterface::Link& link1) {
    m_excludedPairs.erase(LinkPair(&link0, &link1));
    COW *cow0 = GetCow(&link0), *cow1 = GetCow(&link1);
    if (cow0 && cow1) m_allowedCollisionMatrix(cow0->m_index, cow1->m_index) = 1;
  }
  virtual bool RayCastCollision(const Eigen::Vector3d& point1, const Eigen::Vector3d& point2) {
  ...
  virtual void LinksVsAll(const vector<KinBodyInterface::LinkPtr>& links, vector<Collision>& collisions, short filterMask);
  virtual void LinkVsAll(const KinBodyInterface::Link& link, vector<Collision>& collisions, short filterMask);
  virtual void ContinuousCheckTrajectory(const TrajArray& traj, Configuration& rad, vector<Collision>&);
  virtual void CastVsAll(Configuration& rad, const vector<KinBodyInterface::LinkPtr>& links, const DblVec& startjoints, const DblVec& endjoints, vector<Collision>& collisions);

  CollisionObjectWrapper* GetCow(const KinBodyInterface::Link* link) {
    Link2Cow::iterator it = m_link2cow.find(link);
    return (it == m_link2cow.end()) ? 0 : it->second;
  }
  void SetCow(const KinBodyInterface::Link* link, COW* cow) {m_link2cow[link] = cow;}
  void LinkVsAll_NoUpdate(const KinBodyInterface::Link& link, vector<Collision>& collisions, short filterMask);
  void UpdateBulletFromRave();
  void AddKinBody(const KinBodyInterface::Ptr& body);
  void RemoveKinBody(const KinBodyInterface::Ptr& body);
  void AddAndRemoveBodies(const vector<KinBodyInterface::Ptr>& curVec, const vector<KinBodyInterface::Ptr>& prevVec, vector<KinBodyInterface::Ptr>& addedBodies);
...
struct CollisionCollector : public btCollisionWorld::ContactResultCallback {
  ...
  virtual btScalar addSingleResult(btManifoldPoint& cp,
    ...
    const KinBodyInterface::Link* linkA = getLink(colObj0Wrap->getCollisionObject());
    const KinBodyInterface::Link* linkB = getLink(colObj1Wrap->getCollisionObject());
...
BulletCollisionChecker::BulletCollisionChecker(EnvironmentInterface::Ptr env) :
...
void BulletCollisionChecker::SetContactDistance(float dist) {
  ...
  gContactBreakingThreshold = 2.001*dist; 
...
void BulletCollisionChecker::AllVsAll(vector<Collision>& collisions) {
  ...
  for (int i = 0; i < numManifolds; ++i) {
    ...
    for (int j = 0; j < numContacts; ++j) {
      ...
      const KinBodyInterface::Link* bodyA = objA->m_link;
      const KinBodyInterface::Link* bodyB = objB->m_link;

      if (CanCollide(objA, objB)) {
        collisions.push_back(Collision(bodyA, bodyB, toEigen(pt.getPositionWorldOnA()), toEigen(pt.getPositionWorldOnB()),
            toEigen(pt.m_normalWorldOnB), pt.m_distance1, 1./numContacts));
      }
      else {
...
void BulletCollisionChecker::LinksVsAll(const vector<KinBodyInterface::LinkPtr>& links, vector<Collision>& collisions, short filterMask) {
  UpdateBulletFromRave();
  m_world->updateAabbs();
...
void BulletCollisionChecker::LinkVsAll(const KinBodyInterface::Link& link, vector<Collision>& collisions, short filterMask) {
  UpdateBulletFromRave();
  LinkVsAll_NoUpdate(link, collisions, filterMask);
}

void BulletCollisionChecker::LinkVsAll_NoUpdate(const KinBodyInterface::Link& link, vector<Collision>& collisions, short filterMask) {
...
void BulletCollisionChecker::AddKinBody(const KinBodyInterface::Ptr& body) {
  auto cd = std::make_shared<KinBodyCollisionData>(body);

  int filterGroup = body->IsRobot() ? RobotFilter : KinBodyFilter;
  const auto& links = body->GetLinks();
...
void BulletCollisionChecker::RemoveKinBody(const KinBodyInterface::Ptr& body) {
...
void BulletCollisionChecker::AddAndRemoveBodies(const vector<KinBodyInterface::Ptr>& curVec, const vector<KinBodyInterface::Ptr>& prevVec, vector<KinBodyInterface::Ptr>& toAdd) {
  vector<KinBodyInterface::Ptr> toRemove;
...
void BulletCollisionChecker::UpdateAllowedCollisionMatrix() {
  for (const LinkPair& pair : m_excludedPairs) {
    const KinBodyInterface::Link* linkA = pair.first;
    const KinBodyInterface::Link* linkB = pair.second;
...
void BulletCollisionChecker::UpdateBulletFromRave() {
  vector<KinBodyInterface::Ptr> bodies, addedBodies;
  ...
  if (bodies.size() != m_prevbodies.size() || !std::equal(bodies.begin(), bodies.end(), m_prevbodies.begin())) {
    ...
    for (const auto& body : addedBodies) {
...
// void BulletCollisionChecker::PlotCollisionGeometry(vector<shared_ptr<void>>& handles) {
//   UpdateBulletFromRave();
//   btCollisionObjectArray& objs = m_world->getCollisionObjectArray();
//   LOG_DEBUG("%i objects in bullet world", objs.size());
//   for (int i=0; i < objs.size(); ++i) {
//     RenderCollisionShape(objs[i]->getCollisionShape(), objs[i]->getWorldTransform(), *m_env, handles);
//   }
// }
...
void ContinuousCheckShape(btCollisionShape* shape, const vector<btTransform>& transforms,
    KinBodyInterface::Link* link, btCollisionWorld* world, vector<Collision>& collisions) {
...
void BulletCollisionChecker::ContinuousCheckTrajectory(const TrajArray& traj, Configuration& rad, vector<Collision>& collisions) {
  ...
  vector<KinBodyInterface::LinkPtr> links;
...
public:
  ...
  btScalar getMargin() const {return 0;}

  int getNumPreferredPenetrationDirections() const {return 0;}
  void getPreferredPenetrationDirection(int index, btVector3& penetrationVector) const {throw std::runtime_error("not implemented");}

  void calculateLocalInertia(btScalar, btVector3&) const {throw std::runtime_error("not implemented");}
  const char* getName() const {return "CastHull";}
  btVector3 localGetSupportingVertexWithoutMargin(const btVector3& v) const {return localGetSupportingVertex(v);}
...
void BulletCollisionChecker::CastVsAll(Configuration& rad, const vector<KinBodyInterface::LinkPtr>& links,
...
CollisionCheckerPtr CreateCollisionChecker(EnvironmentInterface::Ptr env) {
