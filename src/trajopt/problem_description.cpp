...
RobotAndDOFPtr RADFromName(const string& name, RobotInterface::Ptr robot) {
  if (name == "active") {
  ...
  for (int i = 0; i < components.size(); ++i) {
    ...
    if (RobotInterface::ManipulatorPtr manip = robot->GetManipulatorByName(component)) {
    ...
    } else if (KinBodyInterface::JointPtr joint = robot->GetJoint(component)) {
      dof_inds.push_back(joint->GetDOFIndex());
    } else {
      PRINT_AND_THROW(boost::format("error in reading manip description: %s must be a manipulator, link, or 'base'") % component);
    }
  }
  return RobotAndDOFPtr(new RobotAndDOF(robot, dof_inds, affinedofs, rotationaxis));
}

bool allClose(const Eigen::VectorXd& a, const Eigen::VectorXd& b) {
...
void fromJson(const Json::Value& v, Eigen::Vector3d& x) {
  vector<double> vx;
  fromJsonArray(v, vx, 3);
  x = Eigen::Vector3d(vx[0], vx[1], vx[2]);
}
void fromJson(const Json::Value& v, Eigen::Vector4d& x) {
  vector<double> vx;
  fromJsonArray(v, vx, 4);
  x = Eigen::Vector4d(vx[0], vx[1], vx[2], vx[3]);
}
...
TermInfoPtr TermInfo::fromName(const string& type) {
  ...
  if (name2maker.find(type) != name2maker.end()) {
    ...
    LOG_ERROR("There is no cost of type %s\n", type.c_str());
    return TermInfoPtr();
  }
}
...
void InitInfo::fromJson(const Json::Value& v) {
  ...
  if (type_str == "stationary") {
    ...
    for (int idof = 0; idof < n_dof; ++idof) {
      data.col(idof) = Eigen::VectorXd::LinSpaced(n_steps, start[idof], endpoint[idof]);
    }
  }
}
...
void ProblemConstructionInfo::fromJson(const Value& v) {
  ...
  RobotInterface::Ptr robot = (basic_info.robot == "") ? env->GetRobot() : env->GetRobotByName(basic_info.robot);
  if (!robot) {
...
TrajOptProbPtr ConstructProblem(const Json::Value& root, EnvironmentInterface::Ptr env) {
...
void PoseCostInfo::fromJson(const Value& v) {
  ...
  childFromJson(params, pos_coeffs, "pos_coeffs", (Eigen::Vector3d)Eigen::Vector3d::Ones());
  childFromJson(params, rot_coeffs, "rot_coeffs", (Eigen::Vector3d)Eigen::Vector3d::Ones());
...
void PoseCostInfo::hatch(TrajOptProb& prob) {
  ...
  VectorOfVectorPtr f(new CartPoseErrCalculator(toEigenTransform(wxyz, xyz), prob.GetRAD(), link));
...
void CartVelCntInfo::hatch(TrajOptProb& prob) {
  for (int iStep = first_step; iStep < last_step; ++iStep) {
    prob.addConstraint(ConstraintPtr(new ConstraintFromFunc(
      ...
      concat(prob.GetVarRow(iStep), prob.GetVarRow(iStep + 1)), Eigen::VectorXd::Ones(0), INEQ, "CartVel")));
  }
}
...
void CollisionCostInfo::hatch(TrajOptProb& prob) {
  ...
  CollisionChecker::Ptr cc = CollisionChecker::GetOrCreate(prob.GetEnv());
