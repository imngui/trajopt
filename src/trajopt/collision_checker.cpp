...
CollisionChecker::Ptr CollisionChecker::GetOrCreate(EnvironmentInterface::Ptr env) {
    UserDataPtr ud = GetUserData(env, "trajopt_cc");
    if (!ud) {
        LOG_INFO("creating bullet collision checker for environment");
        ud = CreateCollisionChecker(env);
        SetUserData(env, "trajopt_cc", ud);
    } else {
        LOG_DEBUG("already have a collision checker for this environment");
    }
    return std::dynamic_pointer_cast<CollisionChecker>(ud);
}

#if 0
void CollisionPairIgnorer::ExcludePair(const KinBodyInterface::Link& link1, const KinBodyInterface::Link& link2) {
    m_pairs.insert(LinkPair(&link1, &link2));
    m_pairs.insert(LinkPair(&link2, &link1));
}
bool CollisionPairIgnorer::CanCollide(const KinBodyInterface::Link& link1, const KinBodyInterface::Link& link2) const {
    return m_pairs.find(LinkPair(&link1, &link2)) == m_pairs.end();
}
#endif

void CollisionChecker::IgnoreZeroStateSelfCollisions(KinBodyInterface::Ptr body) {
...
void CollisionChecker::IgnoreZeroStateSelfCollisions() {
    std::vector<KinBodyInterface::Ptr> bodies;
    m_env->GetBodies(bodies);

    BOOST_FOREACH(const KinBodyInterface::Ptr& body, bodies) {
