...
public:
  ...
  virtual void LinkVsAll(const Link& link, std::vector<Collision>& collisions) = 0;
  virtual void LinksVsAll(const std::vector<LinkPtr>& links, std::vector<Collision>& collisions) = 0;
  ...
  static boost::shared_ptr<CollisionChecker> GetOrCreate(EnvironmentPtr env);
};
...
#endif // TRAJOPT_VOXEL_COLLISION_CHECKER_HPP
