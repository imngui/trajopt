...
void CollisionsToDistances(const std::vector<Collision>& collisions, const Link2Int& link2ind, DblVec& dists) {
    dists.clear();
...
void CollisionsToDistanceExpressions(const std::vector<Collision>& collisions, Configuration& rad,
                                     const Link2Int& link2ind, const VarVector& vars, const DblVec& dofvals,
                                     std::vector<AffExpr>& exprs, bool isTimestep1) {
    exprs.clear();
...
void CollisionsToDistanceExpressions(const std::vector<Collision>& collisions, Configuration& rad, const Link2Int& link2ind,
                                     const VarVector& vars0, const VarVector& vars1, const DblVec& vals0, const DblVec& vals1,
                                     std::vector<AffExpr>& exprs) {
    std::vector<AffExpr> exprs0, exprs1;
...
void CollisionEvaluator::GetCollisionsCached(const DblVec& x, std::vector<Collision>& collisions) {
    double key = hash(getDblVec(x, GetVars()));
    std::vector<Collision>* it = m_cache.get(key);
    if (it != NULL) {
...
SingleTimestepCollisionEvaluator::SingleTimestepCollisionEvaluator(ConfigurationPtr rad, const VarVector& vars) :
    ...
    m_links(),
    m_filterMask(-1) {
    std::vector<KinBodyInterface::LinkPtr> links;
    std::vector<int> inds;
...
void SingleTimestepCollisionEvaluator::CalcCollisions(const DblVec& x, std::vector<Collision>& collisions) {
...
void SingleTimestepCollisionEvaluator::CalcDistExpressions(const DblVec& x, std::vector<AffExpr>& exprs) {
    std::vector<Collision> collisions;
    GetCollisionsCached(x, collisions);
    DblVec dofvals = getDblVec(x, m_vars);
...
void CastCollisionEvaluator::CalcCollisions(const DblVec& x, std::vector<Collision>& collisions) {
...
void CastCollisionEvaluator::CalcDistExpressions(const DblVec& x, std::vector<AffExpr>& exprs) {
    std::vector<Collision> collisions;
    GetCollisionsCached(x, collisions);
    DblVec dofvals0 = getDblVec(x, m_vars0);
    DblVec dofvals1 = getDblVec(x, m_vars1);
...
ConvexObjectivePtr CollisionCost::convex(const std::vector<double>& x, Model* model) {
...
double CollisionCost::value(const std::vector<double>& x) {
    DblVec dists;
...
// void CollisionCost::Plot(const DblVec& x, EnvironmentInterface& env, std::vector<std::shared_ptr<void>>& handles) {
//     std::vector<Collision> collisions;
...
ConvexConstraintsPtr CollisionConstraint::convex(const std::vector<double>& x, Model* model) {
...
DblVec CollisionConstraint::value(const std::vector<double>& x) {
    DblVec dists;
    m_calc->CalcDists(x, dists);
...
// void CollisionConstraint::Plot(const DblVec& x, EnvironmentInterface& env, std::vector<std::shared_ptr<void>>& handles) {
//     std::vector<Collision> collisions;
//     m_calc->GetCollisionsCached(x, collisions);
//     PlotCollisions(collisions, env, handles, m_dist_pen);
// }
